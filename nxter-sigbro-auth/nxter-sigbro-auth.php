...
function sigbro_auth__redirect_after_login($redirect_to, $request, $user)
{
    ...
    if ($prefix == 'ARDOR' || $prefix == 'ardor') {
        ...
        if (strlen(get_the_author_meta('sigbro_email', $user->ID)) > 0) {
            ...
            return home_url('/welcome-to-the-ardor-web/', 'relative');
        } else {
            return home_url('/welcome-to-the-ardor-web/', 'relative');
        }
...
wp_register_style('nxtbridge-bootstrap', plugins_url('css/nxtbridge-bootstrap.min.css', __FILE__), '', $random, 'all');
